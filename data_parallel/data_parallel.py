...
def train(model, train_loader , optimizer , epoch, rank , do_data_parallel=False):
    ...
    print(f"Epochs {epoch + 1} - Average Training Loss: {avg_train_loss:.4f} ")
...
def test(model, test_loader, rank):
    ...
    all_true_label_in_epoch = []
    all_predictions_in_epcoh = []
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(test_loader, leave=False):
            ...
            all_predictions_in_epcoh.extend(predicted_labels.tolist())
            all_true_label_in_epoch.extend(labels.tolist())
        ...
        return all_true_label_in_epoch , all_predictions_in_epcoh, accuracy
...
def data_parallel_main(args):
    ...
    # Modify the model architecture to support Imagenette
    ...
    print('Time taken per epochs (seconds): ' + str( ((end_time - start_time).seconds) / epochs ) )    
